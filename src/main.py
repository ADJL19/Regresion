...
def main():
    ...
    with open("hiperparametros.json", "r") as config_file:
    ...
    [target, predictores] = fnc.importacionDatos(config, reduccion= None)
    # # p_train, p_test, t_train, t_test = train_test_split(predictores, target, test_size=0.16667)

    # fnc.crearModelos(config)
    ...
    modelos = {}
    modelos['RL'] = model(LinearRegression())
    # # modelos['KNN'] = model(KNeighborsRegressor(n_neighbors= 30))
    # # modelos['DT'] = model(DecisionTreeRegressor(criterion= 'squared_error', max_depth= 10))
    # # modelos['RF'] = model(RandomForestRegressor(criterion= 'squared_error', max_depth= 10, n_estimators= 50, n_jobs= -1))
    # # modelos['SVM'] = model(SVR())
    ...
    print("SVM00")
    modelosSVM = {}
    modelosSVM['SVM00'] = model(SVR(cache_size= 3000))
    dfM= fnc.predValidacionCruzada(modelosSVM, predictores, target, metricas)
    dfM.to_excel("./info/metricas/SVM/SVM00.xlsx")

    print("SVM01")
    modelosSVM = {}
    modelosSVM['SVM01'] = model(SVR(kernel= 'linear', cache_size= 3000))
    dfM= fnc.predValidacionCruzada(modelosSVM, predictores, target, metricas)
    dfM.to_excel("./info/metricas/SVM/SVM01.xlsx")

    print("SVM02")
    modelosSVM = {}
    modelosSVM['SVM02'] = model(SVR(kernel= 'sigmoid', cache_size= 3000))
    dfM= fnc.predValidacionCruzada(modelosSVM, predictores, target, metricas)
    dfM.to_excel("./info/metricas/SVM/SVM02.xlsx")

    print("SVM10")
    modelosSVM = {}
    modelosSVM['SVM10'] = model(SVR(kernel= 'poly', cache_size= 3000))
    dfM= fnc.predValidacionCruzada(modelosSVM, predictores, target, metricas)
    dfM.to_excel("./info/metricas/SVM/SVM10.xlsx")
    
    print("SVM11")
    modelosSVM = {}
    modelosSVM['SVM11'] = model(SVR(kernel= 'poly', degree= 5, cache_size= 3000))
    dfM= fnc.predValidacionCruzada(modelosSVM, predictores, target, metricas)
    dfM.to_excel("./info/metricas/SVM/SVM11.xlsx")

    print("SVM12")
    modelosSVM = {}
    modelosSVM['SVM12'] = model(SVR(kernel= 'poly', degree= 7, cache_size= 3000))
    dfM= fnc.predValidacionCruzada(modelosSVM, predictores, target, metricas)
    dfM.to_excel("./info/metricas/SVM/SVM12.xlsx")
